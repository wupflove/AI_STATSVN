...
public class RepoDump {
	...
	public void dump() {
		...
		logger.info("----------------------------------");
		logger.info("Current Repo Line Code :" + repository.getCurrentLOC());
		logger.info("-----Via Files--------------------");
		logger.info("Number of Files via Files  :" + files.size());
		logger.info(
		...
		logger.info("# of File Revision via File:" + totalNumRevision);
		logger.info("-----Via Revisions----------------");
		logger.info(
	...
	private void dumpPerFile(final SortedSet files) {
		...
		for (final Iterator it = files.iterator(); it.hasNext();) {
			...
			logger.info("File " + ++fileNumber + "/ " + rev.getFilenameWithPath() + " \tLOC:" + rev.getCurrentLinesOfCode());
			int sumDelta = 0;
	...
	private Set dumpPerRevision(final SortedSet revisions) {
		...
		for (final Iterator it = revisions.iterator(); it.hasNext();) {
			...
			if (!rev.getRevisionNumber().equals(previousRevision)) {
				...
				if (revTotal != -1) {
					logger.info("Total for this rev: " + totalDelta);
				}
				logger.info(
