/*
 * StatCvs - CVS statistics generation Copyright (C) 2002 Lukasz Pekacki <lukasz@pekacki.de>
 * http://statcvs.sf.net/
 * 
 ...
 * 02111-1307 USA
 * 
 * $RCSfile: Main.java,v $ Created on $Date: 2005/03/20 19:12:25 $
 */
package org.jpf.statsvn;
...
import org.jpf.statsvn.input.Builder;
import org.jpf.statsvn.input.RepositoryFileManager;
import org.jpf.statsvn.input.SvnLogfileParser;
import org.jpf.statsvn.output.SvnCommandLineParser;
...
public final class Main {
    ...
    public static void init() {
        Messages.setPrimaryResource("org.jpf.statsvn.statcvs"); // primary is
        // statcvs.properties in net.sf.statsvn

        logger.info(Messages.getString("PROJECT_NAME") + Messages.NL);
    }
    ...
    private static void printProperUsageAndExit() {
        ...
        logger.error(
                ...
                "Usage: java -jar statsvn.jar [options] <logfile> <directory>" + cr + cr + "Required parameters:" + cr
                        ...
                        + "Some options:" + cr + "  -version           print the version information and exit" + cr
                        ...
                        + cr + "  -dump                 dump the Repository content on console" + cr
    ...
    private static void printVersionAndExit() {
        logger.error("Version " + Messages.getString("PROJECT_VERSION"));
        System.exit(1);
    }

    private static void printOutOfMemMessageAndExit() {
        logger.error("OutOfMemoryError.");
        logger.error("Try running java with the -mx option (e.g. -mx128m for 128Mb).");
        System.exit(1);
    }

    private static void printLogErrorMessageAndExit(final String message) {
        logger.error("Logfile parsing failed.");
    ...
    /**
     * Generates HTML report. {@link org.jpf.statsvn.output.SvnConfigurationOptions} must be
     ...
     * initialized before calling this method.
     * 
     * @param repFileMan RepositoryFileManager which is used to access the files in the repository.
     * 
     * @throws LogSyntaxException if the logfile contains unexpected syntax
     * @throws IOException if some file can't be read or written
     * @throws ConfigurationException if a required ConfigurationOption was not set
     */
    public static void generateDefaultHTMLSuite(final RepositoryFileManager repFileMan)
        ...
        logger.info("Parsing SVN log '" + ConfigurationOptions.getLogFileName() + "'"
                + (ConfigurationOptions.getExcludePattern() != null
                        ? " exclude pattern '" + ConfigurationOptions.getExcludePattern() + "'"
                        : "No exclude pattern"));
        ...
        logger.info("Generating report for " + ConfigurationOptions.getProjectName() + " into "
        ...
        logger.info(
                "memory usage After Build: " + (((double) memoryUsedOnEnd - memoryUsedOnStart) / KB_IN_ONE_MB) + " kb");
        ...
        final ReportConfig config = new ReportConfig(content, ConfigurationOptions.getProjectName(),
                ConfigurationOptions.getOutputDir(), ConfigurationOptions.getMarkupSyntax(),
                ConfigurationOptions.getCssHandler(), ConfigurationOptions.getCharSet());
        ...
        if (config.getRepository().getAuthors().size() >= 1) {
            final PageGroup developerPages = new PageGroup(Messages.getString("DEVELOPERS"), false);
            final Iterator it = config.getRepository().getAuthors().iterator();
            while (it.hasNext()) {
                final Author developer = (Author) it.next();
                logger.debug(developer.getName());
                // Page devPage = new DeveloperPageMaker(config, developer).toFile();
    ...
    private static void validate(final ReportConfig config) {
        if (config.getRepository() == null || config.getRepository().getRoot() == null
                || config.getRepository().getDirectories() == null) {
            String cr = System.getProperty("line.separator");
            printErrorMessageAndExit("The repository object is not valid. Please check your settings." + cr
                    + "Possible reasons:" + cr + "1/ Did you use the option -v to create the SVN log" + cr
                    + "2/ Is the log file empty?" + cr + "3/ Do you run from a checked out directory (you should)?" + cr
